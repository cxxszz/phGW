...
from lib.util import array2g_nx
...
def partition_featurize_graph_fpdwl(G, k=100, dims=64, wl_steps=1, distribution_offset=0, distribution_exponent=0):
    """
    ...
    partition_iter = asyn_fluidc(G, k)  # dict

    partition = []
    for i in partition_iter:
        partition.append(i)  # list of sets
    ...
    print("node_subset={}, node_subset_idx={}".format(node_subset, node_subset_idx))
...
def compressed_fgw(dists1, dists2, fdists1, fdists2, membership1, membership2, features1, features2, p1, p2,
                   node_subset_idx1, node_subset_idx2, dists_subset1, dists_subset2,
...
def compress_fgw_from_dicts(data1, data2, alpha=0.1, beta=0.1, verbose=False, return_dense=True):
    ...
    full_coup12 = compressed_fgw(dists1, dists2, fdists1, fdists2,
                                 ...
                                 dists_subset1, dists_subset2,
                                 p_subset1, p_subset2, alpha=alpha, beta=beta, verbose=verbose,
...
def match_qgw(graph_st: graph_pair_rn, dataset="ppi", name="800_900_900_10_4", seed=1, use_cuda=False, ini="uni",
              clu_num=2, partition_level=3, node_limit=200):
    # Parameters
    ...
    start = time.time()
    # ============================ main algorithms ==============================
    gs_nx = array2g_nx(graph_st.graph_s.w)
    gt_nx = array2g_nx(graph_st.graph_t.w)
    assert nx.algorithms.components.is_connected(gs_nx)
    assert nx.algorithms.components.is_connected(gt_nx)
    dataset = partition_featurize_graphlist_fpdwl([gs_nx, gt_nx], k=clu_num ** partition_level, wl_steps=wl_steps,
                                                  distribution_offset=0, distribution_exponent=distribution_exponent)
    trans = compress_fgw_from_dicts(dataset[0], dataset[1], alpha=0.5, beta=0.5, verbose=True, return_dense=True)
    corre = np.argmax(trans, axis=1)
    result = graph_st.result_eval(corre)
    ec = edge_correctness(graph_st, corre)
    s3 = symm_substru_score(graph_st, corre)
    print("result={}, ec={}, s3={}, takes time {}".format(result, ec, s3, time.time() - start))
